...
### Round 54: TPS Optimization + Faction Rivalry Rebalancing

Five fixes addressing P0 (TPS regression) and P1 (no negative sentiment) from the TickWeek 116 health report.

**P0: TPS dropped 47% (0.47→0.25)** after R53 activated the inter-settlement layer. Root cause: three O(n²) settlement-pair patterns in hourly/weekly/daily processing — `resolveMerchantTrade()` (hourly, 41M distance computations/TickWeek), `computeSettlementRelations()` (weekly, 123K pairs), `GetDiplomacyCrimeBonus()` (daily, 3,212 agreements × 496 settlements). Additional bottlenecks: per-tick 400K Fisher-Yates shuffle, per-tick bestProductionHex O(7) for all 152K working agents.

**P1: Zero negative sentiment** despite 496 settlements. Root cause: 6.86× asymmetry in faction scoring — shared faction used Being (1.618) while different factions used Agnosis (0.236). With Verdant Circle at 39% population, most pairs shared VC as dominant faction, creating a positive floor.

233. **Settlement neighbor index** — NEW: `BuildSettlementNeighbors()` in `simulation.go` pre-computes `SettlementNeighbors map[uint64][]*social.Settlement` (≤10 hexes) and `SettlementTradeNeighbors` (≤5 hexes with market). Built once on startup and rebuilt after settlement founding/abandonment. Used by `resolveMerchantTrade()` (direct lookup, no distance filtering), `computeSettlementRelations()` (eliminates all-pairs iteration), and `GetDiplomacyCrimeBonus()` (pre-computed cache rebuilt weekly). Expected ~10-50× reduction in distance computations.
234. **Faction rivalry rebalancing** — FIXED: Two changes in `computeSettlementRelations()` in `relations.go`: (a) Different-faction coefficient changed from Agnosis (0.236) to Matter (0.618). Cooperation:rivalry ratio is now Being/Matter = Φ — the golden ratio itself. (b) New `factionRivalryMultiplier()` returns Being (1.618) for philosophically opposed faction pairs: Crown↔Ashen Path (order vs dissolution), Iron Brotherhood↔Verdant Circle (discipline vs harmony), Merchant's Compact↔Ashen Path (wealth vs detachment). Non-rival different-faction pairs use 1.0× multiplier. Math: IB (34%) vs VC (39%) pair with moderate culture divergence now reaches steady-state sentiment -0.237 — just below the -0.236 warfare threshold.
235. **Hourly agent shuffle** — FIXED: `TickMinute()` Fisher-Yates shuffle reduced from per-tick to per-sim-hour (60× fewer). With fractional extraction (R33), per-tick fairness is no longer critical — agents take partial amounts proportional to availability. Eliminates ~400K swap operations on 59 of every 60 ticks.
236. **bestProductionHex hourly cache** — NEW: `prodHexCache map[prodHexKey]*world.Hex` caches best production hex per (settlement, occupation), refreshed hourly. Hex resource ranking is stable within 1 sim-hour (depletion rate ~0.01/extraction vs ~152K queries/tick). Reduces hex search calls from ~152K/tick to ~4K cache entries/hour. Cache is lazy-filled on first access per key, cleared every sim-hour.
237. **Merchant trade neighbor pre-filter** — NEW: `SettlementTradeNeighbors map[uint64][]*social.Settlement` pre-computes the ≤5 hex market-having subset of the 10-hex neighbor index. Built once alongside `SettlementNeighbors`. Eliminates per-settlement distance filtering that ran every hour for every merchant (~103M distance calculations/day → 0).
